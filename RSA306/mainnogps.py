...
class rsa:
    ...
    def WriteCSV(self):
        try:
            while True:
                ...
                a = c_double(1)
                #self.fwrite(byref(a),4,1,self.f)
                #self.csvwriter = csv.writer(f)
                self.GetIQData()
                s = c_int(1)
                self.fwrite(byref(s),2,1,self.f)
                self.fclose(self.f)
        finally:
            self.fclose(self.f)
    ...
    def GetIQData(self):
        ...
        if ready:
            ...
            print 'Device not ready. Timeout (GetIQdata)' + str(ret)
            sys.exit(1)
        #z = [[iqData[x],iqData[x+1]] for x in xrange(0,self.iqRecordLength,2)]
        #print len(z)
        #self.csvwriter.writerows(z)
        print 'write'
        self.fwrite(iqData,4,self.iqRecordLength,self.f)
        print 'write done'
